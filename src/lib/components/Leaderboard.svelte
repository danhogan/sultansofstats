<script lang="ts">
	...
	$effect(() => {
		orderBy =
			statValueLocation === "statValues" ? "overallRank" : "divisionRank";
		order = "asc";
	});
	...
	<table>
			...
			<tr>
				{#each headCells as cell}
					<th
						...
						class:numeric={cell.numeric && !cell.center}
						class:active={orderBy === cell.id}
					>
		...
		<tbody>
			{#each sortedRows as row (row.teamName + row.leagueName)}
				...
				>
					<td
						style="border-left: 3px solid {getDivisionColor(
							row.division,
						)}"
					>
						<a
							...
							>{row.teamName}</a
						>{#if getPromoIndicator(row.promotion)}<span
								class="promo-indicator"
								title={getPromoTitle(row.promotion)}
								>{getPromoIndicator(row.promotion)}</span
							>{/if}
					</td>
					<td>
						<a
					...
					<td class="right"
						>{overallBool ? row.overallRank : row.divisionRank}</td
					>
					<td class="right"
						>{overallBool
							? row.totalPoints
							: row.divisionPoints}</td
					>

					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool ? row.RRank : row.divisionRRank,
							)}"
							>{overallBool ? row.RRank : row.divisionRRank}</span
						><span class="stat-val">{row.R}</span></td
					>
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool ? row.HRRank : row.divisionHRRank,
							)}"
							>{overallBool
								? row.HRRank
								: row.divisionHRRank}</span
						><span class="stat-val">{row.HR}</span></td
					>
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool ? row.RBIRank : row.divisionRBIRank,
							)}"
							>{overallBool
								? row.RBIRank
								: row.divisionRBIRank}</span
						><span class="stat-val">{row.RBI}</span></td
					>
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool ? row.SBRank : row.divisionSBRank,
							)}"
							>{overallBool
								? row.SBRank
								: row.divisionSBRank}</span
						><span class="stat-val">{row.SB}</span></td
					>
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
					...
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool ? row.KRank : row.divisionKRank,
							)}"
							>{overallBool ? row.KRank : row.divisionKRank}</span
						><span class="stat-val">{row.K}</span></td
					>
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool ? row.K9Rank : row.divisionK9Rank,
							)}"
							>{overallBool
								? row.K9Rank
					...
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
								overallBool
									? row.SVHLDRank
									: row.divisionSVHLDRank,
							)}"
							>{overallBool
								? row.SVHLDRank
								: row.divisionSVHLDRank}</span
						><span class="stat-val">{row.SVHLD}</span></td
					>
					<td class="stat-cell"
						><span
							class="chip chip-sm {getColor(
...
<style>
	...
	tr.row-selected {
		background-color: rgba(237, 75, 37, 0.15);
	}
